...
import type { HookResult, Nuxt, ResolvedNuxtTemplate } from 'nuxt/schema'
...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(passedOptions, nuxt) {
    ...
    const resolvedSchemaPath = rootResolver.resolve(
    ...
    addTemplateOnce(Template.GraphqlConfig, () => {
      const patterns = options.autoImportPatterns || []
      const configPath = rootResolver.resolve(
        (options.graphqlConfigFilePath || '').replace('/graphql.config.ts', ''),
      )
      const schemaPath = './' + relative(configPath, resolvedSchemaPath)
      const documents = patterns
        .filter((v) => !v.includes('!'))
        .map((pattern) => {
          return './' + relative(configPath, rootResolver.resolve(pattern))
        })
      return `
...
`
    ...
    const template = addTemplateOnce(Template.ServerOptions, () => {
...
`
    ...
    addTemplateOnce(Template.ServerOptionsTypes, () => {
...
export { serverOptions }`
    ...
    addTemplateOnce(Template.ClientOptions, () => {
...
export { clientOptions }`
    ...
    addTemplateOnce(Template.ClientOptionsTypes, () => {
      if (clientOptionsImport) {
